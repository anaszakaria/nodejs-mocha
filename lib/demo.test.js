...
var demo = rewire('./demo') // user rewire to access private methods on stub
...
describe('demo', () => {
    ...
    context('test doubles', () => {
        it('should spy on log', () => {
            ...
            demo.foo()
            expect(spy.calledOnce).to.be.true // sinon expect
            expect(spy).to.have.been.calledOnce // sinon-chai expect
            spy.restore()
        })

        it('should stub console.warn', () => {
            let stub = sinon.stub(console, 'warn').callsFake(() => {
                console.log('message from stub') // add our own callback
            })
