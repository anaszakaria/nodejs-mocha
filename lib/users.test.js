...
describe('users', () => {
    ...
    beforeEach(() => {
        ...
        deleteStub = sandbox.stub(mongoose.Model, 'remove').resolves('User removed')
    ...
    context('get', () => {
        ...
        it('should catch error if there is one', done => {
            sandbox.restore()
            let stub = sandbox.stub(mongoose.Model, 'findById').yields(new Error('Something is wrong'))
    ...
    context('create user', () => {
        ...
        beforeEach(async () => {
            ...
            users.__set__('User', FakeUserClass) // rewire method
            result = await users.create(sampleUser)
        })

        it('should reject invalid arguments', async () => {
            await expect(users.create()).to.eventually.be.rejectedWith('Invalid arguments')
            await expect(users.create({ name: 'Anas' })).to.eventually.be.rejectedWith('Invalid arguments')
            await expect(users.create({ email: 'anas@email.com' })).to.eventually.be.rejectedWith('Invalid arguments')
        })
    ...
    context('update user', () => {
        ...
        it('should reject if there is an error', async () => {
            ...
            await expect(users.update(123, { age: 35 })).to.eventually.be.rejectedWith('an error occured')
        })
    })
    ...
    context('reset password', () => {
        ...
        beforeEach(() => {
            resetStub = sandbox.stub(mailer, 'sendPasswordResetEmail').resolves('password reset')
        })
        ...
        it('should call sendPasswordResetEmail', async () => {
            await users.resetPassword('anas@email.com')
            expect(resetStub).to.have.been.calledWith('anas@email.com')
        })
    })
})
